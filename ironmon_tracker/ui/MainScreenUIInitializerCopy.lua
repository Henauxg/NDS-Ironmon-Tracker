...
local function MainScreenUIInitializer(ui, gameInfo)
	...
	function self.initMainFrames()
		...
		ui.frames.mainFrame =
			Frame(
				Box(
					{ x = Graphics.SIZES.SCREEN_WIDTH, y = Graphics.SIZES.SCREEN_HEIGHT },
		...
		for i = 1, 8, 1 do
			local badgeControl =
				ImageLabel(
					...
					ImageField("ironmon_tracker/images/icons/" .. prefix .. "_badge" .. i .. "_OFF.png",
						{ x = -1, y = 0 })
				)
	...
	function self.initHiddenPowerArrows()
		ui.controls.leftHiddenPowerLabel =
			TextLabel(
				Component(ui.frames.hiddenPowerArrowsFrame, Box({ x = 0, y = 0 }, { width = 5, height = 7 }, nil, nil,
					nil)),
				TextField(
					"<",
					{ x = -1, y = -1 },
