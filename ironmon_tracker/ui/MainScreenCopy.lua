local function MainScreen(initialSettings, initialTracker, initialProgram)
	...
	local MainScreenUIInitializer = dofile(Paths.FOLDERS.UI_FOLDER .. "/MainScreenUIInitializerCopy.lua")
	...
	local function onPokemonImageHover(params)
		activeHoverFrame = UIUtils.createAndDrawTypeResistancesFrame(params, program.drawCurrentScreens,
			ui.frames.mainFrame)
	end
	...
	local function onHoverInfo(hoverParams)
		if hoverParams.text ~= "" then
			activeHoverFrame = UIUtils.createAndDrawHoverFrame(hoverParams, program.drawCurrentScreens,
				ui.frames.mainFrame)
		end
	end
	...
	local function setUpMiscInfo(isEnemy)
		...
		local showTourneyPoints = not isEnemy and settings.tourneyTracker.ENABLED and not showAccEva and
			not inPastRunView
	...
	local function initStatListeners()
		for _, stat in pairs(stats) do
			...
			statPredictionEventListeners[stat] =
				MouseClickEventListener(ui.controls[predictionLabel], onStatPredictionClick,
	...
	function self.resetToDefault()
		ui.frames.mainFrame.move({ x = Graphics.SIZES.SCREEN_WIDTH, y = Graphics.SIZES.SCREEN_HEIGHT })
		inTrackedView = false
		inLockedView = false
	end
